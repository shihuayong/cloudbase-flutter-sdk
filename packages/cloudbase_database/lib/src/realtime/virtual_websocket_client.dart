...
import 'package:cloudbase_database/src/config/error_config.dart';
import 'package:cloudbase_database/src/realtime/error.dart';
import 'package:cloudbase_database/src/realtime/listener.dart';
import 'package:cloudbase_database/src/realtime/message.dart';
import 'package:cloudbase_database/src/realtime/snapshot.dart';
import 'package:cloudbase_database/src/realtime/utils.dart';
...
class WatchSessionInfo {
  ...
  WatchSessionInfo({this.queryID, this.currentEventId, this.currentDocs, this.expectEventId});
}
...
class AvailableRetries {
  ...
  AvailableRetries({this.initWatch, this.rebuildWatch, this.checkLast});
...
typedef Future<Map> SendFunc({Map msg, bool waitResponse, bool skipOnMessage, num timeout});
...
class VirtualWebSocketClient {
  ...
  AvailableRetries _availableRetries;
  Timer _ackTimer;
  ...
  VirtualWebSocketClient({
    ...
    this.watchId = 'watchid_${DateTime.now().millisecondsSinceEpoch}_${Random().nextDouble()}';
    this._envId = envId;
  ...
  Future<void> _initWatch({bool forceRefreshLogin, Completer completer}) async {
  ...
  void _internalInitWatch(bool forceRefreshLogin, Completer completer) async {
    try {
      ...
      LoginResult loginResult = await this._internalLogin(envId: this._envId, refresh: forceRefreshLogin);
      ...
      this._handleWatchEstablishmentError(e: e, operationName: 'INIT_WATCH', completer: completer);
    }
  }

  Future<void> _rebuildWatch({bool forceRefreshLogin, Completer completer}) async {
  ...
  void _internalRebuildWatch(bool forceRefreshLogin, Completer completer) async {
    try {
      if (this._watchStatus == WATCH_STATUS.PAUSED) {
        Console.log('[realtime] initWatch cancelled on pause');
        return completer.complete();
      ...
      this._handleWatchEstablishmentError(e: e, operationName: 'REBUILD_WATCH', completer: completer);
    }
  }


  void _handleWatchEstablishmentError({e, String operationName, Completer completer}) async {
    bool isInitWatch = operationName == 'INIT_WATCH';

    var abortWatch = () {
      this.closeWithError(CloudBaseException(
        code: isInitWatch ? ErrCode.SDK_DATABASE_REALTIME_LISTENER_INIT_WATCH_FAIL : ErrCode.SDK_DATABASE_REALTIME_LISTENER_REBUILD_WATCH_FAIL,
        message: e.toString()
      ));
      completer.completeError(e);
    };

    var retry = ([bool refreshLogin]) async {
      if (this._useRetryTicket(operationName)) {
        if (isInitWatch) {
          this._initWatchPromise = null;
          await this._initWatch(forceRefreshLogin: refreshLogin, completer: completer);
          completer.complete();
        } else {
          this._rebuildWatchPromise = null;
          await this._rebuildWatch(forceRefreshLogin: refreshLogin, completer: completer);
          completer.complete();
        }
      } else {
        abortWatch();
      ...
      onUnknownError: () async {
        try {
          ...
          if (this._isWSConnected() != true) {
            ...
            await Future.delayed(Duration(milliseconds: DEFAULT_WAIT_TIME_ON_UNKNOWN_ERROR));
            if (this._watchStatus == WATCH_STATUS.PAUSED) {
              completer.completeError(CancelledException(message: '$operationName cancelled due to pause after unknownError'));
  ...
  Future<void> _closeWatch() async {
    String queryId = this._sessionInfo != null ? this._sessionInfo.queryID : '';
  ...
  void _scheduleSendACK() {
    ...
    this._ackTimer = Timer(Duration(milliseconds: DEFAULT_SEND_ACK_DEBOUNCE_TIMEOUT), () {
  ...
  Future<void> _handleCommonError(e, {
    onSignError,
    onTimeoutError,
    onCancelledError,
    onNotRetryableError,
  ...
  bool _useRetryTicket(String operationName) {
    ...
    if (retryTimes != null && retryTimes > 0) {
      ...
      Console.log('[realtime] $operationName use a retry ticket, now only $retryTimes retry left');
  ...
  Future<void> _handleServerEvents(Map msg) async {
    try {
      ...
      Console.error('[realtime listener] internal non-fatal error: handle server events failed with error: ${e.toString()}');
  ...
  Future<void> _internalHandleServerEvents(Map msg) async {
    ...
    try {
      ...
      throw CloudBaseException(
        code: ErrCode.SDK_DATABASE_REALTIME_LISTENER_RECEIVE_INVALID_SERVER_DATA,
        message: e.toString()
      );
    }
    ...
    for (var i = 0, len = allChangeEvents.length; i < len; i++) {
      ...
      if (sessionInfo.currentEventId >= change.id) {
        if (allChangeEvents[i-1] == null || change.id > allChangeEvents[i-1].id) {
        ...
        switch(change.dataType) {
          case 'update': {
            if (change.doc == null) {
              switch(change.queueType) {
                case 'update':
                case 'dequeue': {
                  var localDoc = docs.firstWhere((doc) => doc['_id'] == change.docId, orElse: () => null);
                  if (localDoc != null) {
                ...
                case 'enqueue': {
                  ...
                  CloudBaseException exception = CloudBaseException(
                    code: ErrCode.SDK_DATABASE_REALTIME_LISTENER_UNEXPECTED_FATAL_ERROR,
                    message: 'HandleServerEvents: full doc is not provided with dataType="update" and queueType="enqueue" (requestId $requestId)'
                  );
          ...
          case 'replace': {
            ...
            if (change.doc == null) {
              ...
              CloudBaseException exception = CloudBaseException(
                code: ErrCode.SDK_DATABASE_REALTIME_LISTENER_UNEXPECTED_FATAL_ERROR,
                message: 'HandleServerEvents: full doc is not provided with dataType="replace" (requestId $requestId)'
              );
              this.closeWithError(exception);
              throw exception;
            }
          ...
          case 'remove': {
            var doc = docs.firstWhere((doc) => doc['_id'] == change.docId, orElse: () => null);
            if (doc != null) {
              change.doc = doc;
            } else {
            ...
            if (change.doc == null) {
              switch(change.queueType) {
                ...
                case 'enqueue': {
                  ...
                  CloudBaseException exception = CloudBaseException(
                      code: ErrCode.SDK_DATABASE_REALTIME_LISTENER_UNEXPECTED_FATAL_ERROR,
                      message: 'HandleServerEvents: full doc is not provided with dataType="limit" and queueType="enqueue" (requestId $requestId)'
                  );
                  this.closeWithError(exception);
                  throw exception;
                }
        ...
        switch(change.queueType) {
          ...
          case 'dequeue': {
            num index = docs.indexWhere((doc) => doc['_id'] == change.docId);
            if (index > -1) {
              docs.removeAt(index);
            } else {
        ...
        if (i == len -1 || (allChangeEvents[i+1] != null && allChangeEvents[i+1].id != change.id)) {
          ...
          List docChanges = allChangeEvents.sublist(0, i+1);
        ...
        // out-of-order event
        Console.warn('[realtime listener] event received is out of order, cur ${this._sessionInfo.currentEventId} but got ${change.id}');
        // rebuild watch
  ...
  void _postHandleServerEventsValidityCheck(Map msg) {
    ...
    if (this._sessionInfo.expectEventId != null &&
        this._sessionInfo.currentEventId >= this._sessionInfo.expectEventId
    ) {
  ...
  void onMessage(msg) {
    ...
    switch(this._watchStatus) {
      ...
      case WATCH_STATUS.REBUILDING: {
        Console.warn('[realtime listener] internal non-fatal error: unexpected message received while ${this._watchStatus}');
        return;
      }
...
//        Console.warn('nextevent ${msg['msgData']['currEvent']} ignored, msg: $msg}');
      ...
      case 'CHECK_EVENT': {
        if (this._sessionInfo.currentEventId < msg['msgData']['currEvent']) {
          ...
          this._clearWaitExpectedEvent();
          this._waitExpectedTimer = Timer(Duration(milliseconds: this._getWaitExpectedTimeoutLength()), () {
            // must rebuild watch
            this._rebuildWatch();
          });

          Console.log('[realtime] waitExpectedTimeoutLength ${this._getWaitExpectedTimeoutLength()}');
        }
        break;
      }
      case 'ERROR': {
        ...
        this.closeWithError(CloudBaseException(
            ...
            message: '${msg['msgData']['code']} - ${msg['msgData']['message']}'
        ));
        break;
      }
      default: {
        Console.warn('[realtime listener] virtual client receive unexpected msg: $msg');
        break;
      }
    }
  }
  ...
  void closeWithError(error) {
    ...
    String queryID = this._sessionInfo != null && this._sessionInfo.queryID != null ? this._sessionInfo.queryID : '';
    this._onWatchClose(this, queryID);

    Console.log('[realtime] client closed (${this._collectionName} ${this._query}) (watchId ${this.watchId})');
  }

  void pause() {
    this._watchStatus = WATCH_STATUS.PAUSED;

    Console.log('[realtime] client paused (${this._collectionName} ${this._query}) (watchId ${this.watchId})');
  }
  ...
  Future<void> resume() async {
    ...
    this._sessionInfo != null ? 'REBUILD_WATCH' : 'INIT_WATCH'
    } (${this._collectionName} ${this._query}) (${this.watchId})');
    ...
    try {
      ...
      Console.error('[realtime] client resume failed (${this._collectionName} ${this._query}) (${this.watchId}), error: ${e.toString()}');
    }
  }
}