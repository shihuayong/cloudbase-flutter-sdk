...
import 'package:cloudbase_database/src/realtime/virtual_websocket_client.dart';
import 'package:cloudbase_database/src/realtime/ws_channel.dart';
import 'package:cloudbase_database/src/realtime/ws_event.dart';
...
class LoginInfo {
  ...
  LoginInfo({this.loggedIn, this.loggingInPromise, this.loginStartDT, this.loginResult});
}

class LoginResult {
  String envId;

  LoginResult({this.envId});
}
...
class RealtimeWebSocketClient {
  ...
  // appConfig
  num _realtimePingInterval;
  num _realtimePongWaitTimeout;
  GetAccessTokenFunc _getAccessToken;

  RealtimeWebSocketClient({
    num maxReconnect = DEFAULT_MAX_RECONNECT,
    num reconnectInterval = DEFAULT_WS_RECONNECT_INTERVAL,
    num realtimePingInterval,
    num realtimePongWaitTimeout,
  ...
  Future<void> _initWebSocketConnection({bool reconnect = false, num availableRetries = 0}) async {
    // 当前处于正在重连中的状态
  ...
  void _internalInitWebSocketConnection({bool reconnect, num availableRetries, Completer completer}) async {
    try {
      ...
      Console.error('[realtime] initWebSocketConnection connect fail, error: ${e.toString()}');
      ...
      if (availableRetries > 0) {
        ...
        completer.complete(this._initWebSocketConnection(reconnect: reconnect, availableRetries: availableRetries));
      } else {
  ...
  Future<void> _initWebSocketEvent() async {
      ...
      if (!wsOpened) {
        ...
        this._virtualWSClient.forEach((client) {
          client.closeWithError(CloudBaseException(
            code: ErrCode.SDK_DATABASE_REALTIME_LISTENER_WEBSOCKET_CONNECTION_ERROR,
      ...
      switch (code) {
        ...
        case CloseEventCode.AbnormalClosure: {
          ...
          if (this._maxReconnect > 0) {
            this._initWebSocketConnection(reconnect: true, availableRetries: this._maxReconnect);
          } else {
    ...
    var onMessage = (String rawMsg) {
      ...
      if (msgType == 'ERROR') {
        // 找到当前监听，并将error返回
        VirtualWebSocketClient virtualWatch = this._virtualWSClient.firstWhere((item) => (item.watchId == watchId));
      ...
      if (responseWaitSpec != null) {
        try {
          if (msgType == 'ERROR') {
            responseWaitSpec.reject(RealtimeErrorMessageException(serverErrorMsg: msg));
          } else {
            responseWaitSpec.resolve(msg);
          }
        } catch (e) {
          Console.error('ws onMessage responseWaitSpec.resolve(msg) errored: ${e.toString()}');
        } finally {
      ...
      if (msgType == 'PONG') {
        if (this._lastPingSendDT != null) {
          num rtt = DateTime.now().millisecondsSinceEpoch - this._lastPingSendDT.millisecondsSinceEpoch;
          ...
          if (this._rttObserved.length >= MAX_RTT_OBSERVED) {
            this._rttObserved.removeRange(0, this._rttObserved.length - MAX_RTT_OBSERVED + 1);
      ...
      VirtualWebSocketClient client = watchId != null ? this._watchIdClientMap[watchId] : null;
      if (client != null) {
        client.onMessage(msg);
      } else {
        Console.error('[realtime] no realtime listener found responsible for watchId $watchId: $msg');
        switch (msgType) {
          case 'INIT_EVENT':
          case 'NEXT_EVENT':
          case 'CHECK_EVENT':{
  ...
  num _getWaitExpectedTimeoutLength() {
    ...
    // 1.5 * RTT
    return 1.5 * (
        this._rttObserved.reduce((acc, cur) => acc + cur) / this._rttObserved.length
    );
  }
  ...
  _heartbeat([bool immediate = false]) {
    ...
    this._pingTimer = Timer(
        Duration(milliseconds: immediate ? 0 : this._realtimePingInterval), () async {
          try {
            ...
            this._pongTimer = Timer(
                Duration(milliseconds: this._realtimePongWaitTimeout), () {
